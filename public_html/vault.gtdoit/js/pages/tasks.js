...
function renderTasks(){
  ...
  const renderT = t=>`<div class="full-task-row ${t.done?'done':''}">
    ...
    <div class="task-txt" onclick="editTask(${t.id})" style="cursor:pointer;flex:1">
      ...
      <div class="task-meta">${t.category?'🏷 '+esc(t.category):''}${t.due_date?' 📅 '+fmtDate(t.due_date):''}${t.assigned_name?' 👤 '+esc(t.assigned_name):''}</div>
    </div>
...
function openTask(){
  ...
  document.getElementById('ti-prio').value='medium'; document.getElementById('ti-id').value='';
...
async function toggleTask(id){
  ...
  A.tasks=A.tasks.map(x=>x.id==id?u:x); badges(); render(A.page);
  if(!t.done) toast('🎉 Uppgift avklarad!');
}
